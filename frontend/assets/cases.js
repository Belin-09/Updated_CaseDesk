...
async function loadCases() {
  if (selectedYear === null) {
    ...
    if (pageTitle) pageTitle.textContent = "Cases";
    if (yearGrid) yearGrid.style.display = "grid";
    if (casesListContainer) casesListContainer.style.display = "none";
    await loadYears();
  } else {
    if (breadcrumbBar) {
      breadcrumbBar.style.display = "flex";
      breadcrumbYear.textContent = selectedYear;
    }
    if (pageTitle) pageTitle.textContent = `Cases for ${selectedYear}`;
    if (yearGrid) yearGrid.style.display = "none";
    if (casesListContainer) casesListContainer.style.display = "block";
    await fetchCasesForYear();
  }
}

async function loadYears() {
  try {
    totalCount.textContent = "Loading folders...";
    const response = await apiFetch("/cases/years");
    if (!response || !response.ok) {
      totalCount.textContent = "Error loading folders";
      return;
    }
    ...
    totalCount.textContent = `${totalAllCount} cases found across ${years.length} years`;

    if (years.length === 0) {
      yearGrid.innerHTML = `
        <div style="grid-column: 1/-1; text-align: center; padding: 40px; color: #8a93a3;">
          <div style="font-size: 32px; margin-bottom: 10px;">📁</div>
          No year folders found. Please run a scan.
        </div>
      `;
      return;
    }
    ...
    yearGrid.innerHTML = years.map(y => {
      const displayYear = y.year === 'Unknown' ? 'Unknown Year' : y.year;
      return `
        <div class="year-folder-card" onclick="selectYearFolder('${y.year}')">
          <div class="folder-icon-wrapper">
            <svg class="folder-svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#4f9cff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path>
            </svg>
          </div>
          <div class="folder-details">
            <div class="folder-name">${escapeHtml(displayYear)}</div>
            <div class="folder-count">${y.count} case${y.count !== 1 ? 's' : ''}</div>
          </div>
        </div>
      `;
    }).join("");
...
function selectYearFolder(year) {
  selectedYear = year;
  currentPage = 1;
  loadCases();
}

async function fetchCasesForYear() {
  try {
